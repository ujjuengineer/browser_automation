...
# Assuming log, PAGE_TIMEOUT_MS, NAV_TIMEOUT_MS, and UPLOAD_TIMEOUT_MS are defined globally
...
def find_and_process_volume(page: Page, volume_no: str, volume_district: str) -> None:
    ...
    for _ in range(20):
        ...
        if row:
            ...
            # FIX: Wait for network to quiet down and the specific file inputs to attach.
            # This ensures listDetails() AJAX has completed constructing the new table.
            try:
...
def upload_pdf_files(page: Page, folder_path: str) -> None:
    ...
    for iteration in range(500):  # safety cap
        # FIX: Explicitly wait for file inputs to be present rather than just generic table rows.
        # This prevents the loop from racing ahead while listDetails() clears/re-renders HTML.
        try:
            page.wait_for_selector(
                "#table-section table tbody tr input[type='file']", 
                state="attached", 
            ...
            # If no file inputs exist, either all are uploaded or the table is genuinely empty.
        ...
        # FIX: The execution triggering the dialog must happen INSIDE the expect_event block context!
...
def submit_uploaded_volume(page: Page) -> None:
    ...
    # FIX: Avoid using page.on("dialog") listeners across asynchronous wait_for_url states.
    # Instead, handle sequential dialog confirmations cleanly with explicit context blocks.
    try:
        # 1. Capture the initial confirmation Dialog ("Are you sure you want to submit?")
        # OR capture the validation error dialog if the upload was missed ("Please upload all deed documents.")
        ...
        first_dialog.accept() # Accepts the "Are you sure?" confirmation prompt

        # 2. Capture the server success response Dialog ("The volume has been submitted for verification.")
        try:
        ...
        # 3. Wait for the browser redirect to finish execution
        page.wait_for_url(
